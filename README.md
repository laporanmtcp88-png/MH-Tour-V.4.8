...
Versi 4.7 Hardened ini dirancang untuk komunikasi audio **tanpa internet** dan telah diperiksa ulang pada sisi Android, LiveKit, token server, jaringan hotspot, izin Android, dan GitHub Actions. Semua HP Guide/Jemaah harus tersambung ke satu hotspot Wi‑Fi lokal. Server lokal menjalankan LiveKit + token service.
...
- Debug: `MH-Tour-APK` — untuk pengujian instalasi.
- Workflow uji tidak membutuhkan keystore atau GitHub Secrets.
...
Build CI fix: AndroidX is explicitly enabled in `android/gradle.properties`, and the GitHub Actions workflow uses a valid `workflow_dispatch` trigger. The test workflow builds both Debug and a temporarily CI-signed Release APK without requiring GitHub Secrets.

## Pemeriksaan sebelum upload GitHub

Windows:
```bat
tools\preflight-check.bat
```
...
Jika muncul `PREFLIGHT OK`, lanjutkan upload ke GitHub dan jalankan workflow **MH Tour APK Build** secara manual melalui **Actions → Run workflow**.
